...
const Growl = ({
  ...
  if (variant === "success") {
    ...
    statusIcon = (
      <AlertIcon fill="var(--token-warning-500)" height={40} width={40} />
    )
  } else if (variant === "error") {
    statusIcon = (
      <AlertIcon fill="var(--token-error-500)" height={40} width={40} />
    )
  }
  ...
  return (
    <div className={cx(styles.growl, styles[variant], className, { expanded })}>
      ...
      <div
        className={cx(styles.collapsed, { expanded })}
        onClick={() => setExpanded(!expanded)}
      >
        <div className={styles.text__wrapper}>
          <div className={styles.time}>{timeString}</div>
          <div className={styles.hash}>{truncate(txHash, [6, 6])}</div>
        </div>
        <div
        ...
        >
          <CloseIcon
            ...
            width={12}
            stroke="var(--token-dark-900)"
            className={cx(styles.close__icon, {
              [styles.display__close]: showClose,
            })}
            onClick={handleClose}
          />
          <div
            className={cx(styles.status__icon, {
              [styles.no__close]: !showClose,
            })}
          >
            {statusIcon}
          </div>
